...
<xsl:stylesheet xmlns:exist="http://exist.sourceforge.net/NS/exist"
	...
	xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:pulfa="http://library.princeton.edu/pulfa"
	xmlns:xlink="http://www.w3.org/1999/xlink" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"
	xmlns:lib="http://findingaids.princeton.edu/pulfa/2/lib" xmlns:mix="http://www.loc.gov/mix/v20"
  ...
  -->
	...
	<xsl:template match="/">
		<html xmlns="http://www.w3.org/1999/xhtml">
			...
			<body>
				...
				<script language="Javascript" type="text/javascript" src="js/jquery-1.12.3.min.js"><!--
						#//--></script>
				<script language="Javascript" type="text/javascript" src="js/local.js"><!--
						#//--></script>
			</body>
		</html>
	</xsl:template>
	...
	<xsl:template name="menu">
		<ul id="menu">
				...
				<li>
					<a href="{$xslt.base-uri}/index.html#bioghist">
						<xsl:choose>
							...
							<xsl:when
								test="//ead:archdesc/ead:did/ead:origination/(ead:persname | ead:famname)">
								<xsl:text>Biography</xsl:text>
							</xsl:when>
							<xsl:otherwise>
								<xsl:choose>
									<xsl:when
										test="//ead:archdesc/ead:did/ead:origination/ead:corpname">
			...
			<xsl:for-each
				select="
					//ead:archdesc/ead:dsc/(ead:c01[@level = 'series'] | ead:c[@level = 'series']) |
...
<xsl:call-template name="otherprocessinfo">
		...
		<dl>
			...
			<xsl:if test="ead:physdesc/ead:extent">
				...
				<xsl:if test="ead:physdesc[@altrender = 'part']/ead:extent">
					<dd class="archdescData">
						<xsl:for-each
							select="ead:physdesc[@altrender = 'part'][not(position() = last())]/ead:extent">
							<xsl:apply-templates/>
							<xsl:text>, </xsl:text>
						</xsl:for-each>
						<xsl:apply-templates
							select="ead:physdesc[@altrender = 'part'][last()]/ead:extent"/>
					</dd>
				</xsl:if>
			</xsl:if>
			...
			<xsl:if test="ead:physloc">
				...
				<xsl:for-each select="ead:physloc">
					...
					<xsl:if test="not(matches(., '\s')) and not(../ead:physloc[matches(., '\s')])">
						<xsl:for-each select="current()">
							<xsl:choose>
								<xsl:when
									test="
										matches(., '^rcpph$') or matches(., '^rcppf$') or matches(., '^rcpxg$') or matches(., '^rcpxm$')
										or matches(., '^rcpxr$') or matches(., '^rcppa$') or matches(., '^rcpxc$')">
									<dd class="archdescData">This collection is stored offsite at
										the ReCAP facility.</dd>
								</xsl:when>
								<xsl:otherwise>
									<xsl:choose>
										<xsl:when
											test="
												matches(., '^flm$') or matches(., '^flmp$') or matches(., '^wa$') or matches(., '^gax$')
												or matches(., '^mss$') or matches(., '^ex$') or matches(., '^flmm$')
												or matches(., '^ctsn$') or matches(., '^thx$')">
											<dd class="archdescData">This collection is stored
												onsite at Firestone Library.</dd>
										</xsl:when>
										<xsl:otherwise>
											<xsl:choose>
												<xsl:when
												test="matches(., '^hsvc$') or matches(., '^hsvg$') or matches(., '^hsvm$') or matches(., '^hsvr$')">
												<dd class="archdescData">This collection is stored
												in special vault facilities at Firestone
												Library.</dd>
												</xsl:when>
												<xsl:otherwise>
												<xsl:choose>
												<xsl:when test="matches(., '^anxb$')">
												<dd class="archdescData">This collection is stored
												offsite at Annex B (Fine Hall).</dd>
												</xsl:when>
												<xsl:otherwise>
												<xsl:choose>
												<xsl:when test="matches(., '^st$')">
												<dd class="archdescData">This collection is stored
												onsite at the Engineering Library.</dd>
												</xsl:when>
												<xsl:otherwise>
												<xsl:choose>
												<xsl:when test="matches(., '^ppl$')">
												<dd class="archdescData">This collection is stored
												onsite at the Plasma Physics Library.</dd>
												</xsl:when>
												</xsl:choose>
												</xsl:otherwise>
												</xsl:choose>
	...
	<xsl:template match="ead:archdesc/ead:arrangement">
		...
		<xsl:choose>
			...
			<xsl:otherwise>
				<xsl:if
					test="//ead:c[@level = 'series' or @level = 'subseries'] | //ead:c01[@level = 'series'] | //ead:c02[@level = 'subseries']">
					<ul class="normalIndent">
						<xsl:for-each
							select="//ead:c[@level = 'series' or @level = 'subseries'] | //ead:c01[@level = 'series'] | //ead:c02[@level = 'subseries']">
							<li>
								<a>
...
If only unitdate or only unittitle, display as is.  -->
								...
								<td class="container">
									<xsl:for-each select="ead:did/ead:container">
										<xsl:choose>
											<xsl:when test="ead:ptr">
												...
												select="ead:ptr/@target"/>
												<xsl:value-of
												select="concat(lib:capitalize-first(//ead:dsc[@type = 'othertype']/ead:c[@id = $target_id]/ead:did/ead:container/@type), ' ', //ead:dsc[@type = 'othertype']/ead:c[@id = $target_id]/ead:did/ead:container)"/>
												<xsl:if test="position() ne last()">,</xsl:if>
											</xsl:when>
											<xsl:when
												test="./@parent and ./@parent[not(matches(., '^cid'))] and //ead:dsc[@type = 'othertype']">
												<xsl:variable name="sub_container" select="@parent"/>
												<xsl:value-of
												select="upper-case(substring(//ead:dsc[@type = 'othertype']/ead:c[@id = $sub_container]/ead:did/ead:container/@type, 1, 1))"/>
												<xsl:value-of
												select="substring(//ead:dsc[@type = 'othertype']/ead:c[@id = $sub_container]/ead:did/ead:container/@type, 2)"/>
												<xsl:text> </xsl:text>
												<xsl:value-of
                                    ...
                                    <xsl:value-of select="./concat(normalize-space(upper-case(@type)), ' ', normalize-space(.))"/>
											...
											<xsl:otherwise>
												...
												<xsl:value-of
												select="
															concat(upper-case(substring(pul:strip-trailspace(@type), 1, 1)),
										...
										<xsl:text> </xsl:text>
										<xsl:if
											test="not(.[@type = 'album'] | .[@type = 'box'] | .[@type = 'carton'] | .[@type = 'case'] | .[@type = 'letterbook'] | .[@type = 'notebook'] | .[@type = 'oversize'] | .[@type = 'package'] | .[@type = 'portfolio'] | .[@type = 'scrapbook'] | .[@type = 'tube'] | .[@type = 'volume'])">
											<xsl:value-of
												select="./normalize-space(upper-case(substring(@type, 1, 1)))"/>
											<xsl:value-of
												select="./concat(normalize-space(substring(@type, 2)), ' ', normalize-space(.))"
											/>
										</xsl:if>
									</xsl:for-each>
									<xsl:for-each select="ead:did/ead:unitid[@type = 'itemnumber']">
										...
										<xsl:choose>
											<xsl:when test="ead:ptr">
												...
												select="ead:ptr/@target"/>
												<xsl:value-of
												select="concat(upper-case(//ead:dsc[@type = 'othertype']/ead:c[@id = $target_id]/ead:did/ead:container/@type), ' ', //ead:dsc[@type = 'othertype']/ead:c[@id = $target_id]/ead:did/ead:container)"/>
												<xsl:if test="position() ne last()">,</xsl:if>
											</xsl:when>
											<xsl:otherwise>
												<xsl:value-of select="."/>
			...
			<xsl:when test="ead:extent[not(@type)] | ead:dimensions | ead:physfacet">
				<xsl:if test="ead:extent[not(@type)]">
					<p>Size: <xsl:apply-templates select="ead:extent[not(@type)][position() = 1]"/>
						<xsl:if test="ead:extent[not(@type)][position() > 1]"><xsl:for-each
								select="ead:extent[not(@type)][position() > 1]">,
	...
	<xsl:template match="ead:langmaterial[ancestor::ead:dsc]">
		...
		<p>
			...
			<xsl:for-each select="ead:language[position() > 1]">, <xsl:apply-templates
			...
			<xsl:otherwise>
				<ul class="normalIndent">
					<xsl:for-each select="ead:item">
                        ...
                        -->
								<xsl:if
									test="ead:ref[@altrender = 'series'] | ead:ref[@altrender = 'subgrp']">
	...
	<xsl:template match="ead:p" mode="#all">
		<p>
			<xsl:if
				test="
					parent::ead:dsc//ead:accessrestrict | parent::ead:dsc//ead:userestrict |
